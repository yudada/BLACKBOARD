  ...
  <div>
    <el-card>
      ...
      <el-row>
        <el-col class="student_box">
          <div v-for="(student, index) in studentList" :key="index" class="student_card">
            <div class="student_content">
              ...
              <div class="student_score">
                <el-tag type="success" size="medium" @click="addScoreMark(student.sid)">{{ student.addScore }}</el-tag>
                <el-tag type="danger" size="medium" @click="minusScoreMark(student.sid)">{{ student.minusScore }}</el-tag>
              </div>
              <el-checkbox
    ...
    <div class="classroom_mark">
      <div class="mark_btn">
        ...
        <el-button @click="checkedBoxDialog = !checkedBoxDialog">多选</el-button>
        <el-button v-show="checkedBoxDialog === true"  @click="openRewardsDialog">评分</el-button>
      </div>
    </div>
    <!-- 评分弹框 -->
    <el-dialog
      ...
      <div>
        <div class="tag_dialog">
          <el-tag effect="dark" type="success" v-for="item in tagAddList" :key="item.id" size="medium" @click="handleTagPlus(item)">{{item.scoreName}}{{item.scoreNum}}</el-tag>
        </div>
        <div class="tag_dialog">
          <el-tag effect="dark" type="danger" v-for="item in tagMinusList" :key="item.id" size="medium" @click="handleTagMinus(item)">{{item.scoreName}}{{item.scoreNum}}</el-tag>
        </div>
        <div class="tag_dialog">
          <el-tag effect="dark" type="warning" v-for="item in tagAllList" :key="item.id" size="medium" @click="handleTagAll(item)">{{item.scoreName}}{{item.scoreNum}}</el-tag>
        </div>
      </div>
      ...
      <div class="content_dialog">
        <div class="topbox_dialog">
          <div class="op_content" v-show="changeName === true">
            <el-button type="text" icon="el-icon-minus" @click="numMinus"></el-button>
            <el-input v-model="selectNum"></el-input>
            <el-button type="text" icon="el-icon-plus" @click="numPlus"></el-button>
          </div>
        ...
        <div class="btn_content">
          <el-button type="success" @click="startSelect">{{ btnText }}</el-button>
          <el-button type="info" @click="changeSelectNum">切换人数</el-button>
          <el-button type="warning" @click="openRewardsDialog" v-show="!hidenMark">评分</el-button>
        </div>
      </div>
    </el-dialog>
  </div>
    ...
    startSelect() {
      if (this.intervalId) {
        ...
        this.intervalId = setInterval(() => {
          this.selectedStudentIndexs = this.studentIndexs.sort(() => 0.5 - Math.random()).slice(0, this.selectNum)
          }, 100)
      }
    },
    // 点名结果打分
    setMarjList() {
      ...
      for (let item of this.selectedStudentIndexs) {
        this.studentList.forEach((item2, keys) => {
          if(keys === item) {
            this.markList.push(item2.sid),
            this.sidArr.push(item2.stuName)
          }
        })
      }
    },
