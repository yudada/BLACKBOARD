      ...
      <el-row>
        <el-col :span="24">
          <el-card class="box-card" v-if="schoolRuleForm">
            ...
            <el-form :model="schoolRuleForm" :rules="schoolRuleFormRules" ref="schoolRuleFormRef">
              ...
              <!-- 富文本编辑器组件 -->
              <quill-editor v-model="schoolRuleForm.schoolIntroduce"></quill-editor>
              ...
              :auto-upload="false">
                <img v-if="this.schoolRuleForm.schoolLogo" :src="this.schoolRuleForm.schoolLogo" class="avatar">
                <el-button v-else size="small" type="text">点击上传</el-button>
              </el-upload>
              <div class="goNext_btn">
  ...
  methods: {
    ...
    submitForm() {
      ...
      this.$refs.schoolRuleFormRef.validate(async valid => {
        if(!valid) return
        console.log(this.schoolRuleForm);
        const {data: res} = await this.$http.post(`api/school/update`, this.schoolRuleForm)
        if (res.statusCode !== 200) return this.$message.error(res.msg)
      })
    }
  }
