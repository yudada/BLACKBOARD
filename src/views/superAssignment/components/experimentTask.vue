<template>
  <div class="resource_box">
    ...
    <el-dialog
      :title="dialogContent.expName"
      :visible.sync="dialogVisible"
      width="80%"
      :append-to-body="true"
      custom-class="modle_dialog"
    >
      <iframe :src="dialogContent.expAddress" class="content_box" />
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    modelDialogVisible(item) {
      console.log(item);
      if(!item.expAddress) return this.$message.info('暂无内容')
